...
public class OVRGrabber : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider otherCollider)
    {
        ...
        Debug.Log($"Registered {grabbable.name}, Count: {m_grabCandidates[grabbable]}");
    }

    void OnTriggerExit(Collider otherCollider)
    {
        Debug.Log($"OnTriggerExit: {otherCollider.name}");
        OVRGrabbable grabbable = otherCollider.GetComponent<OVRGrabbable>() ??
                                 otherCollider.GetComponentInParent<OVRGrabbable>();
        if (grabbable == null) return;
    ...
    {
        // 掴んでいるオブジェクトが存在する場合のみ処理
        if (m_grabbedObj != null)
        {
            // 特定のオブジェクト（例: 銃）を手放さない処理
            if (m_grabbedObj.CompareTag("gun")) // 銃オブジェクトに"Gun"タグを設定
            {
                Debug.Log("Gun is grabbed and won't be released.");
                return; // 銃の場合は掴み解除をスキップ
            }
